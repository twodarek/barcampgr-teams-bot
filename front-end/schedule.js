...
$(function() {
    ...
    $("#grid-refresh").on("click", function(event) {
        event.preventDefault();
        populateSchedule(sample4);
    });
});
    ...
    for (var track of data["rows"]) {
        if (track["sessions"] != null) {
            for (var session of track["sessions"]) {
                ...
                var targetCell = $(targetCellSelector);
                debugger;
                var hasContent = (targetCell.find(".title").length > 0);

                if (hasContent) {
                    // update the stuff
                    if (session.title == 'Blocked' && !session.speaker) {
                        targetCell.find(".title").remove();
                        targetCell.find(".speaker").remove();
                        targetCell.html(targetCell.html() + "<img src='red-x.png' class='blocked-slot' alt='big red x marking this slot as blocked, probably because there is something important happening in a different room at the same time'>");
                    } else {
                        targetCell.find(".title").text(session.title);
                        targetCell.find(".speaker").text(session.speaker);
                    }
                } else {
                    if (session.title == 'Blocked' && !session.speaker) {
                        targetCell.html(targetCell.html() + "<img src='red-x.png' class='blocked-slot' alt='big red x marking this slot as blocked, probably because there is something important happening in a different room at the same time'>");
                    } else {
                        targetCell.html(targetCell.html() + "<span class='title'>" + session.title + "</span><span class='speaker'>" + session.speaker + "</span>");
                    }
                }

                targetCell.find(".a11y-pxl").attr("alt", session.altText);
                targetCell.attr("aria-description", session.altText);
            }
        }
    }
}
